        ...
        if user_database_id:
            ...
            for file_data in files_data.get('results', []):
                try:
                    ...
                    parent_folder = '/' + '/'.join(folder_path.strip('/').split('/')[:-1]) + ('/' if folder_path != '/' else '')
                    if is_folder and parent_folder == current_folder:
                        folder_entries.append({
                            "name": folder_path.strip('/').split('/')[-1] or '/',
                    ...
                    continue
        return render_template('home.html', files=files, folders=folder_entries, nav_folders=sorted(nav_folders), current_folder=current_folder)
    except Exception as e:
...
def update_file_metadata():
    try:
        ...
        uploader.update_file_metadata(file_id, filename=new_name, folder_path=new_folder, file_hash=file_hash)
